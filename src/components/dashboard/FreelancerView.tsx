...
import { 
  ...
  Send, MessageCircle, Share2, ThumbsUp} from "lucide-react";
...
export const FreelancerView = ({ jobs, userMetadata }: { jobs: any[], userMetadata: any }) => {
  ...
  const handleSupportEmail = () => {
    ...
    window.location.href = `mailto:support.nexusgigs@gmail.com?subject=Help&body=Msg: ${supportMsg}%0D%0AID: ${user?.id}`;
  };

  const marketplaceGigs = useMemo(() => [
    { id: "1", title: "Business: Cloud Network Audit", budget: 850, client: "TechFlow Systems", rating: 5.0, dur: "4 Days", img: "https://i.pravatar.cc/150?u=tech", type: "Company", status: "Active" },
    { id: "2", title: "School: Advanced Math Solutions", budget: 60, client: "Mark Thompson", rating: 4.8, dur: "12 Hours", img: "https://i.pravatar.cc/150?u=mark", type: "Academic", status: "Active" },
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="relative w-full max-w-90 bg-[#0a0f1e] border-2 border-white/10 rounded-[45px] p-10 text-center shadow-2xl overflow-hidden">
              ...
              {paymentStep === "terms" ? (
                ...
                <div className="space-y-6 text-sm">
                  ...
                  <div className="space-y-3">
                    <button onClick={() => handleSecurePayment("CARD")} className="w-full py-4 bg-white text-black font-black rounded-2xl text-[10px] uppercase italic flex items-center justify-center gap-3 tracking-widest shadow-xl active:scale-95 transition-all">
                      <CreditCard size={14}/> Card
                    </button>
                    <button onClick={() => setPaymentStep("mpesa")} className="w-full py-4 bg-emerald-600 text-white font-black rounded-2xl text-[10px] uppercase italic flex items-center justify-center gap-3 tracking-widest shadow-lg active:scale-95 transition-all">
