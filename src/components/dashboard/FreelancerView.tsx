...
export const FreelancerView = ({ jobs, userMetadata }: { jobs: any[], userMetadata: any }) => {
  ...
  // --- 🛰️ INTASEND TIER-1 UPLINK ($7.00 / KES 910) ---
  const handleIntasendPayment = async (method: "M-PESA" | "CARD") => {
    if (method === "M-PESA" && mpesaNumber.length < 10) return alert("Invalid Phone Format. Use 254...");
    ...
    try {
      ...
      alert("📡 SIGNAL INTERRUPTED: Check Vercel Keys.");
      setIsPaying(false);
    }
  };
  ...
  const marketplaceGigs = useMemo(() => [
    { id: "1", title: "Next.js Performance Audit", budget: 450, client: "Alpha Tech", rating: 4.9, dur: "2 Days", img: "https://i.pravatar.cc/150?u=1", closed: false },
    { id: "3", title: "UI Engine Optimization", budget: 300, client: "Nexa Studio", rating: 4.8, dur: "5 Hours", img: "https://i.pravatar.cc/150?u=3", closed: false },
    { id: "4", title: "Cyber Security Scan", budget: 1500, client: "Defi Pulse", rating: 5.0, dur: "7 Days", img: "https://i.pravatar.cc/150?u=4", closed: false },
    { id: "12", title: "Global Relay Setup", budget: 550, client: "Drop Ship", rating: 5.0, dur: "6 Days", img: "https://i.pravatar.cc/150?u=12", closed: false },
  ], []);
  ...
  return (
    <div className="max-w-6xl mx-auto pb-44 pt-4 px-4 text-white relative font-sans selection:bg-[#00f2ff]/30">
      ...
      {activeTab === "home" && (
        <div className="space-y-6 animate-in fade-in duration-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-8 bg-linear-to-br from-[#0a0f1e] to-[#00f2ff]/10 border border-[#00f2ff]/30 rounded-[45px] shadow-2xl">
              ...
              <button onClick={() => setActiveTab('earnings')} className="w-full mt-6 py-3 bg-white text-black font-black rounded-2xl text-[9px] uppercase hover:bg-[#00f2ff] transition-all">Settle Funds</button>
            </div>
          ...
          {!isVerified && (
            <div className="p-10 bg-[#00f2ff]/5 border border-[#00f2ff]/20 rounded-[50px] group relative overflow-hidden">
      ...
      {/* --- 💼 TAB: MISSION FEED (Functional Bidding) --- */}
      {activeTab === "tasks" && (
            ...
            {marketplaceGigs.map(g => (
              <div key={g.id} className="p-8 rounded-[45px] bg-white/3 border border-white/10 hover:border-[#00f2ff]/40 group transition-all duration-500 shadow-2xl">
                ...
                <div className="flex justify-between items-center pt-6 border-t border-white/5">
                   ...
                   <button onClick={() => !isVerified ? setShowVerifyModal(true) : alert("Bid Dispatched to Secure Relay")} className="px-5 py-2 bg-white text-black text-[9px] font-black uppercase rounded-xl hover:bg-[#00f2ff] transition-all">Bid Mission</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {activeTab === "contracts" && (
        <div className="space-y-8 animate-in fade-in">
          ...
          <div className="p-12 bg-white/2 border border-white/10 rounded-[50px] text-center space-y-6">
             <div className="w-20 h-20 bg-amber-500/10 border border-amber-500/20 rounded-full flex items-center justify-center mx-auto text-3xl text-amber-500 shadow-2xl italic font-black">!</div>
             <p className="text-gray-500 text-sm max-w-sm mx-auto italic font-medium leading-relaxed">No active missions detected. Your account status is currently [UNVERIFIED]. Please complete the sync to unlock work history.</p>
        ...
        <div className="h-[60vh] flex border border-white/10 rounded-[50px] overflow-hidden bg-black/40 animate-in fade-in">
           ...
           <main className="flex-1 p-8 flex flex-col justify-between">
              <div className="space-y-6">
                 <div className="max-w-[80%] bg-white/5 border border-white/10 p-6 rounded-3xl rounded-tl-none italic">
      ...
      {/* --- 💰 TAB: WALLET (Functional Conversion) --- */}
      {activeTab === "earnings" && (
        <div className="space-y-10 animate-in fade-in">
           <div className="flex justify-between items-center border-b border-white/5 pb-4">
              ...
              <button onClick={() => setCurrency(currency === "USD" ? "KES" : "USD")} className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-[8px] font-black uppercase italic hover:bg-white hover:text-black transition-all">Toggle {currency === "USD" ? "KES" : "USD"}</button>
           </div>
           <div className="grid md:grid-cols-2 gap-6">
              <div className="p-14 bg-linear-to-br from-[#00f2ff]/10 to-transparent border border-[#00f2ff]/20 rounded-[60px] text-center shadow-2xl">
                 <p className="text-[10px] font-black text-[#00f2ff] uppercase italic tracking-widest mb-4">Available for Settle</p>
                 <h4 className="text-6xl font-black italic mb-2 tracking-tighter">{currency === "USD" ? "$0.00" : "KES 0"}</h4>
              </div>
              <div className="p-10 bg-white/2 border border-white/10 rounded-[60px] space-y-4">
                 {['M-Pesa Withdrawal', 'Bank Transfer', 'Web3 Relay'].map(m => (
                    <div key={m} onClick={handleWithdrawal} className="p-6 bg-black/40 border border-white/5 rounded-3xl flex justify-between items-center group cursor-pointer hover:bg-white/5">
      ...
      {/* --- 📊 TAB: STATS (Real Data Mapping) --- */}
      {activeTab === "analytics" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in zoom-in-95">
           <div className="p-10 bg-white/5 border border-white/10 rounded-[50px] flex flex-col justify-center items-center text-center">
              <div className="flex items-end gap-3 h-32 mb-8">
                 {[20, 45, 30, 85, 60, 40].map((h, i) => <div key={i} className="w-4 bg-[#00f2ff]/30 rounded-t-lg group hover:bg-[#00f2ff] transition-all" style={{ height: `${h}%` }} />)}
              </div>
              <p className="text-[10px] font-black uppercase italic text-gray-500 tracking-widest">Growth Velocity (Projected)</p>
           </div>
           <div className="grid grid-cols-1 gap-4">
              {[
                { l: "Profile Reach", v: "14.2k", g: "+8%" },
                { l: "Bid Success Rate", v: "88%", g: "+2%" },
                { l: "Avg Payout Sync", v: "0.02s", g: "TOP 1%" }
              ].map(stat => (
                <div key={stat.l} className="p-6 bg-white/5 border border-white/10 rounded-3xl flex justify-between items-center group hover:border-[#00f2ff]/20">
      ...
      {activeTab === "account" && (
        <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in">
           <div className="p-12 bg-black border border-white/10 rounded-[60px] flex flex-col items-center text-center shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 w-full h-1 bg-[#00f2ff]/20" />
              <img src={user?.imageUrl} className="w-28 h-28 rounded-full border-4 border-[#00f2ff]/10 mb-8 shadow-2xl" alt="P" />
              <h3 className="text-3xl font-black italic uppercase tracking-tighter mb-2">{user?.fullName}</h3>
              <p className="text-[10px] font-black text-gray-500 uppercase italic tracking-widest mb-10">Freelance Node Specialist</p>
              <div className="w-full flex flex-wrap gap-2 justify-center pt-8 border-t border-white/5">
                 {['Next.js', 'React', 'TS', 'Node.js', 'Solidity'].map(s => <span key={s} className="px-4 py-2 bg-white/5 rounded-xl text-[9px] font-black uppercase italic border border-white/5">{s}</span>)}
              </div>
           </div>
        </div>
      )}
      ...
      {activeTab === "settings" && (
        <div className="max-w-md mx-auto space-y-4 animate-in fade-in">
           {['Encryption Protocol', 'Uplink Notifications', 'Privacy Matrix', 'Satellite Config'].map(s => (
              <div key={s} className="p-6 bg-white/5 border border-white/10 rounded-3xl flex justify-between items-center group cursor-pointer hover:bg-white/10">
                 <span className="text-[10px] font-black uppercase text-gray-400 group-hover:text-white italic tracking-widest">{s}</span>
                 <span className="text-[#00f2ff] font-black">›</span>
              </div>
           ))}
           <div className="pt-10">
              <SignOutButton><button className="w-full py-5 bg-red-900/10 border border-red-500/20 text-red-500 font-black rounded-3xl text-[10px] uppercase italic tracking-[0.4em] hover:bg-red-500/10">Kill Session</button></SignOutButton>
           </div>
        </div>
      )}
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-100 w-[94%] max-w-2xl h-20 bg-[#0a0f1e]/90 backdrop-blur-3xl border border-white/10 rounded-full shadow-2xl flex justify-around items-center px-4">
        {navItems.map((item) => (
          <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center p-3 rounded-2xl transition-all duration-300 ${activeTab === item.id ? 'bg-[#00f2ff] text-black scale-110 shadow-lg shadow-[#00f2ff]/20' : 'text-gray-500 hover:text-white'}`}>
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 z-200 flex items-center justify-center p-6 backdrop-blur-3xl">
          ...
          <div className="relative w-full max-w-sm bg-[#0a0f1e] border border-white/10 rounded-[50px] p-12 text-center animate-in zoom-in-95 shadow-2xl">
            {paymentStep === "choice" ? (
              <div className="space-y-6">
                <h3 className="text-xl font-black italic uppercase text-[#00f2ff] mb-2 tracking-tighter">ACTIVATE <span className="text-white">NODE</span></h3>
                <p className="text-[10px] text-gray-500 font-black italic mb-8 uppercase tracking-widest">Select Payment Protocol</p>
                ...
                <button onClick={() => setPaymentStep("mpesa")} className="w-full py-5 bg-emerald-600 text-white font-black rounded-2xl uppercase text-[11px] italic shadow-xl">📱 M-PESA UPLINK</button>
                <button onClick={() => setShowVerifyModal(false)} className="text-[9px] text-gray-700 font-black uppercase italic pt-4">Abort Sync</button>
              </div>
            ) : paymentStep === "mpesa" ? (
              <div className="space-y-6">
                ...
                <input value={mpesaNumber} onChange={e => setMpesaNumber(e.target.value)} placeholder="2547XXXXXXXX" className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-center text-sm font-black outline-none focus:border-emerald-500 text-white tracking-[0.3em]" />
                <button disabled={isPaying} onClick={() => handleIntasendPayment("M-PESA")} className="w-full py-5 bg-emerald-600 text-white font-black rounded-2xl uppercase text-[11px] italic">
                   {isPaying ? "ENCRYPTING..." : "PAY KES 910 ($7.00)"}
                </button>
                <button onClick={() => setPaymentStep("choice")} className="text-[9px] text-gray-500 font-black uppercase italic">Back</button>
              </div>
            ) : (
              <div className="space-y-6">
                ...
                <button disabled={isPaying} onClick={() => handleIntasendPayment("CARD")} className="w-full py-5 bg-[#00f2ff] text-black font-black rounded-2xl uppercase text-[11px] italic">
