            ...
            <motion.div key="home" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
              <header className="flex justify-between items-center bg-white/5 backdrop-blur-xl p-5 rounded-[25px] border border-white/10 shadow-2xl relative overflow-hidden">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase text-gray-500 tracking-[0.2em]">Node Operator</p>
                ...
                <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-2xl border border-white/5 shadow-inner">
                   <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2 }} className={`w-2.5 h-2.5 rounded-full ${isVerified ? 'bg-[#00f2ff] shadow-[0_0_10px_#00f2ff]' : 'bg-red-500 shadow-[0_0_10px_red]'}`} />
                   <span className="text-[9px] font-black uppercase tracking-widest">{isVerified ? "AUTHENTICATED" : "SYNC PENDING"}</span>
                </div>
              </header>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {/* Visual Performance Widget */}
                <div className="p-8 bg-linear-to-br from-[#00f2ff]/20 to-transparent border border-[#00f2ff]/30 rounded-[40px] shadow-3xl relative overflow-hidden group">
                  <div className="absolute top-4 right-4 opacity-20 group-hover:rotate-12 transition-transform"><TrendingUp size={40} className="text-[#00f2ff]"/></div>
                  <p className="text-[10px] font-black uppercase text-[#00f2ff] mb-2 tracking-[0.2em]">Current Vault</p>
                  <h3 className="text-4xl font-black italic mb-6 tracking-tighter">$0.00</h3>
                  <div className="w-full h-1 bg-white/10 rounded-full overflow-hidden mb-6 shadow-inner">
                    <motion.div initial={{ width: 0 }} animate={{ width: "30%" }} className="h-full bg-white shadow-glow" />
                  </div>
                  <button onClick={() => setActiveTab("earnings")} className="w-full py-3.5 bg-white text-black font-black rounded-2xl text-[10px] uppercase hover:bg-[#00f2ff] transition-all shadow-xl active:scale-95 tracking-widest">Open Assets</button>
                </div>
                
                {/* Mission Status Widget */}
                <div className="md:col-span-2 p-8 bg-white/5 border border-white/10 rounded-[40px] flex flex-col justify-between backdrop-blur-md relative overflow-hidden shadow-2xl">
                    <div className="flex justify-between items-start mb-4">
                      <div className="space-y-1">
                        <h4 className="text-lg font-black italic uppercase tracking-tighter">Mission Progress</h4>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Verification screening required</p>
                      </div>
                      <div className="p-3 bg-[#00f2ff]/10 rounded-2xl text-[#00f2ff]"><Layers size={20}/></div>
                    </div>
                    
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                       {[
                         { l: "Jobs", v: "0", i: <Target size={14}/> },
                         { l: "Sync", v: "40%", i: <Activity size={14}/> },
                         { l: "Trust", v: "Lvl 1", i: <Shield size={14}/> },
                         { l: "Score", v: "0.0", i: <Star size={14}/> }
                       ].map((item, idx) => (
                         <div key={idx} className="bg-black/20 p-4 rounded-2xl border border-white/5">
                            <div className="text-gray-500 mb-1">{item.i}</div>
                            <p className="text-xs font-black">{item.v}</p>
                            <p className="text-[7px] font-bold text-gray-600 uppercase">{item.l}</p>
                         </div>
                       ))}
                    </div>

                    {!isVerified && (
                       <button onClick={() => { setPaymentStep("terms"); setShowVerifyModal(true); }} className="flex items-center justify-center gap-3 w-full py-4 bg-[#00f2ff]/10 border border-[#00f2ff]/30 text-[#00f2ff] rounded-2xl text-[10px] font-black uppercase italic tracking-[0.2em] hover:bg-[#00f2ff] hover:text-black transition-all group">
                          Start Verification Handshake <ChevronRight size={14} className="group-hover:translate-x-1 transition-transform" />
                       </button>
                    )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 {[
                   { l: "New Missions", i: <Briefcase size={18}/>, t: "tasks" },
                   { l: "Secure Vault", i: <Wallet size={18}/>, t: "earnings" },
                   { l: "Global Stats", i: <BarChart3 size={18}/>, t: "analytics" },
                   { l: "Help Terminal", i: <LifeBuoy size={18}/>, t: "support" }
                 ].map((act, i) => (
                   <button key={i} onClick={() => setActiveTab(act.t)} className="p-6 bg-white/2 border border-white/5 rounded-[30px] flex flex-col items-center gap-3 hover:bg-white/5 transition-all shadow-lg group">
                      <div className="p-3 bg-white/5 rounded-xl text-gray-500 group-hover:text-[#00f2ff] transition-colors">{act.i}</div>
                      <span className="text-[8px] font-black uppercase tracking-widest">{act.l}</span>
                   </button>
                 ))}
              </div>
            </motion.div>
          ...
          {activeTab === "account" && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-3xl mx-auto space-y-6 pb-10">
               ...
               <div className="flex justify-center pt-4">
                  <SignOutButton><button className="px-10 py-5 bg-red-500/5 border-2 border-red-500/10 text-red-500 font-black italic rounded-[30px] uppercase text-[10px] hover:bg-red-500 hover:text-white transition-all tracking-[0.5em] shadow-2xl active:scale-95">Terminate Session</button></SignOutButton>
               </div>
            </motion.div>
          )}
        ...
        {customAlert.show && (
          <div className="fixed inset-0 z-300 flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
             <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-75 bg-[#0a0f1e] border-2 border-white/10 rounded-[40px] p-8 text-center shadow-3xl relative">
                <div className={`absolute top-0 left-0 w-full h-1.5 ${customAlert.type === 'error' ? 'bg-red-500' : 'bg-emerald-500'}`} />
