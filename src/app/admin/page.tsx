...
import { 
  ...
  Fingerprint, MousePointer2, RefreshCw
} from "lucide-react";
...
export default function AdminPage() {
  ...
  const handleCommand = async (actionFn: Function, id: string, extra?: any) => {
    if (confirm("EXECUTE SYSTEM OVERRIDE?")) {
      setIsFetching(true);
      const res = await actionFn(id, extra);
      if (res.success) {
        await syncRegistry();
      } else {
        alert("Command Failure: " + res.message);
      }
      setIsFetching(false);
    }
  };
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6 font-sans">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md bg-black border-2 border-red-500/20 p-16 rounded-[70px] text-center shadow-3xl">
          <Lock size={60} className="mx-auto mb-10 text-red-600" />
          <h1 className="text-3xl font-black italic text-white uppercase mb-8">NEXUS <span className="text-red-600">HQ</span></h1>
          <form onSubmit={(e) => { e.preventDefault(); if(passInput === "Nexus123!") { sessionStorage.setItem("nexus_admin_session", "true"); setIsAuthorized(true); } else { alert("Denied."); } }} className="space-y-6">
    ...
    <div className="flex min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden">
      ...
      <aside className="fixed inset-y-0 left-0 w-80 bg-black border-r border-white/5 hidden lg:flex flex-col">
        ...
        <nav className="px-8 space-y-4 flex-1">
          <button onClick={() => setActiveTab("dashboard")} className={`w-full flex items-center gap-6 px-8 py-6 rounded-[30px] transition-all ${activeTab === "dashboard" ? 'bg-red-600' : 'text-gray-500 hover:bg-white/5'}`}>
            <BarChart size={20}/> <span className="text-[11px] font-black uppercase tracking-widest">Pulse</span>
          </button>
          <button onClick={() => setActiveTab("nodes")} className={`w-full flex items-center gap-6 px-8 py-6 rounded-[30px] transition-all ${activeTab === "nodes" ? 'bg-red-600' : 'text-gray-500 hover:bg-white/5'}`}>
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white p-10 rounded-[50px] shadow-3xl group">
              ...
              <div className="bg-white p-10 rounded-[50px] shadow-3xl group">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">Total Nodes</p>
                <h4 className="text-4xl font-black text-slate-950 tracking-tighter">{metrics.nodeCount}</h4>
              </div>
              <div className="bg-[#0a0f1e] border-2 border-white/10 p-10 rounded-[50px] shadow-3xl group">
        ...
        {activeTab === "nodes" && (
          <div className="space-y-12">
             <div className="flex flex-col md:flex-row justify-between items-center gap-10">
                ...
                <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="FILTER NODES..." className="bg-white/5 border-2 border-white/10 rounded-full px-12 py-5 text-xs font-black text-white w-full md:w-96 outline-none focus:border-red-600 transition-all uppercase italic tracking-widest shadow-2xl" />
             </div>
                ...
                {nodes.filter(n => n.name.toLowerCase().includes(searchTerm.toLowerCase())).map((node) => (
                  <motion.div key={node.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-10 bg-white/3 border-2 border-white/5 rounded-[60px] hover:border-red-600/40 transition-all group relative overflow-hidden shadow-3xl">
                    ...
                    <div className="mb-10">
                       ...
                       <p className="text-[12px] font-black text-[#00f2ff] italic mt-4">PAID: KES {node.paidBalance || 0}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                       <button onClick={() => handleCommand(verifyUserNode, node.id)} className="py-4 bg-emerald-600 text-white rounded-2xl text-[9px] font-black uppercase italic hover:bg-emerald-700 transition-all shadow-xl">Verify</button>
                       <button onClick={() => handleCommand(suspendUserNode, node.id, !node.banned)} className="py-4 bg-amber-600 text-white rounded-2xl text-[9px] font-black uppercase italic hover:bg-amber-700 transition-all shadow-xl">{node.banned ? 'Restore' : 'Suspend'}</button>
                       <button onClick={() => handleCommand(terminateUserNode, node.id)} className="col-span-2 py-4 bg-red-600 text-white rounded-2xl text-[9px] font-black uppercase italic hover:bg-red-700 transition-all shadow-xl">Kill Connection</button>
                    </div>
                  </motion.div>
                ))}
             </div>
