...
import { 
  ...
  ShieldCheck, RefreshCw, ChevronRight, AlertTriangle
} from "lucide-react";
...
export default function AdminPage() {
  ...
  // --- 📈 REVENUE MATRIX (Syncs Balance) ---
  const metrics = useMemo(() => {
    // Summing paidBalance from all user nodes
  ...
  useEffect(() => {
    if (isAuthorized) {
      syncRegistry();
      const heartbeat = setInterval(syncRegistry, 20000);
  ...
  if (!isAuthorized) {
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-md bg-black border-2 border-red-500/20 p-16 rounded-[70px] text-center shadow-3xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if(passInput === "Nexus123!") { sessionStorage.setItem("nexus_admin_session", "true"); setIsAuthorized(true); } }} className="space-y-6">
            ...
            <button className="w-full py-6 bg-red-600 text-white font-black rounded-3xl uppercase text-[12px] italic tracking-widest">Initialize</button>
          </form>
        </motion.div>
      </div>
    );
    ...
    <div className="flex min-h-screen bg-[#020617] text-white font-sans overflow-x-hidden">
      ...
      <aside className="fixed inset-y-0 left-0 w-80 bg-black border-r border-white/5 hidden lg:flex flex-col">
        ...
        <nav className="px-8 space-y-4">
          <button onClick={() => setActiveTab("dashboard")} className={`w-full flex items-center gap-6 px-8 py-6 rounded-[30px] transition-all ${activeTab === "dashboard" ? 'bg-red-600 text-white' : 'text-gray-500 hover:bg-white/5'}`}>
            <BarChart size={20}/> <span className="text-[11px] font-black uppercase tracking-widest">Pulse</span>
          </button>
          <button onClick={() => setActiveTab("nodes")} className={`w-full flex items-center gap-6 px-8 py-6 rounded-[30px] transition-all ${activeTab === "nodes" ? 'bg-red-600 text-white' : 'text-gray-500 hover:bg-white/5'}`}>
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white p-10 rounded-[50px] shadow-3xl transition-transform hover:scale-105">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2">Vault Revenue</p>
              ...
              <div className="bg-[#0a0f1e] border-2 border-white/10 p-10 rounded-[50px] shadow-3xl transition-transform hover:scale-105">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-2 text-red-500">Today Inflow</p>
        ...
        {activeTab === "nodes" && (
          <div className="space-y-12">
             <div className="flex flex-col md:flex-row justify-between items-center gap-10">
                ...
                <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="FILTER..." className="bg-white/5 border-2 border-white/10 rounded-full px-12 py-5 text-xs font-black text-white w-full md:w-96 outline-none focus:border-red-600 transition-all uppercase italic shadow-2xl" />
             </div>
                ...
                {nodes.filter(n => n.name.toLowerCase().includes(searchTerm.toLowerCase())).map((node) => (
                  <motion.div key={node.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-10 bg-white/3 border-2 border-white/5 rounded-[60px] hover:border-red-600/40 transition-all group relative overflow-hidden shadow-3xl">
                    <div className="flex justify-between items-start mb-8">
                       <div className="w-16 h-16 rounded-[25px] bg-white/5 flex items-center justify-center font-black text-2xl text-red-600 border border-white/10">{node.name.charAt(0)}</div>
                       <span className={`px-4 py-2 rounded-full text-[8px] font-black uppercase border ${node.status === 'Verified' ? 'border-emerald-500 text-emerald-500' : 'border-red-600 text-red-600'}`}>{node.status}</span>
                    </div>
                    <div className="mb-10">
                       ...
                       <p className="text-[14px] font-black text-[#00f2ff] italic mt-6 uppercase">Sync Balance: KES {node.paidBalance || 0}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                       <button onClick={() => handleCommand(verifyUserNode, node.id)} className="py-4 bg-emerald-600 text-white rounded-2xl text-[9px] font-black uppercase italic shadow-xl">Verify</button>
                       <button onClick={() => handleCommand(terminateUserNode, node.id)} className="py-4 bg-red-600 text-white rounded-2xl text-[9px] font-black uppercase italic shadow-xl">Kill</button>
                    </div>
                  </motion.div>
                ))}
             </div>
